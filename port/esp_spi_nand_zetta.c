...
static const char *TAG = "uffs_zetta";

// Zetta ZD35 series
// Often compatible with GigaDevice (using 0x70 mask) ??
// Or Winbond (0x30 mask?).
// ZD35Q1GV datasheet (if applicable):
// Status Reg-1 (0xC0)
// Bit 4-6 (ECC):
// 000: No error
// 001: 1-3 corrected
// 010: Uncorrectable (Fail) (Matches Winbond 'Fail' value but 3 bits?)
// 011: 4 corrected
// 100: 5 corrected
// ...
// Actually often Zetta follows GD style (counts errors) or just 2 bits.
// Since User linked espressif/spi_nand_flash, and they support Zetta ZD35Q1GC.
// Let's assume generic logic (ReadPageGeneric uses 2 bits, 0x30).
// If Zetta uses 3 bits, we might mask out the high bit and get confused.
// Safest is to check Zetta specifically.
// Looking at other drivers: Zetta often uses same ECC Status as GigaDevice
// (0x70 mask). 000: Clean, 001-110: Corrected, 111: Fail. Let's assume
// GigaDevice style ECC for Zetta to be safe/advanced.

static int uffs_zetta_init_flash(struct uffs_DeviceSt *dev) {
...
static int uffs_zetta_read_page(struct uffs_DeviceSt *dev, uint32_t block,
  ...
  // Zetta/GD Style Check (Mask 0x70)
  int ecc_res = UFFS_FLASH_NO_ERR;
  int ecc_stat = (status & 0x70) >> 4;

  if (ecc_stat == 7) { // 111b Uncorrectable
...
esp_err_t uffs_spi_nand_init_zetta(struct uffs_DeviceSt *dev,
  ...
  ops->InitFlash = uffs_zetta_init_flash;
  ops->ReadPage = uffs_zetta_read_page;
  ops->WritePage = uffs_spi_nand_write_page_generic;
  ops->WritePageWithLayout = uffs_zetta_write_page_with_layout;
