...
void uffs_InitGlobalFsLock(void) {
  uffs_Perror(UFFS_MSG_NOISY, "[Utils] InitGlobalLock");
...
void uffs_GlobalFsLockUnlock(void) { uffs_SemSignal(_global_lock); }
...
static void _ForceFormatAndCheckBlock(uffs_Device *dev, int block) {
  ...
  if (buf == NULL) {
    uffs_Perror(UFFS_MSG_SERIOUS, "Alloc page buffer fail ! Format stoped.");
    goto ext;
  }
  ...
  for (i = 0; i < dev->attr->pages_per_block; i++) {
    if (ops->WritePageWithLayout)
      ret = ops->WritePageWithLayout(dev, block, i, buf->header,
                                     dev->com.pg_size, NULL, &ts);
    else
      ret = ops->WritePage(dev, block, i, buf->header, dev->com.pg_size, spare,
                           dev->attr->spare_size);
  ...
  for (i = 0; i < dev->attr->pages_per_block; i++) {
    ...
    if (ops->ReadPageWithLayout) {
      ret = ops->ReadPageWithLayout(dev, block, i, buf->header,
                                    dev->com.pg_size, NULL, &ts, NULL);
      if (UFFS_FLASH_IS_BAD_BLOCK(ret))
        goto bad_out;
      ...
      ret = ops->ReadPage(dev, block, i, buf->header, dev->com.pg_size, NULL,
                          spare, dev->attr->spare_size);
      if (UFFS_FLASH_IS_BAD_BLOCK(ret))
        goto bad_out;
      for (j = 0; j < dev->com.pg_size; j++)
        if (buf->header[j] != 0)
...
URET uffs_FormatDeviceEx(uffs_Device *dev, UBOOL force, UBOOL lock) {
  ...
  if (dev->ref_count > 1 && !force) {
    uffs_Perror(UFFS_MSG_NORMAL, "can't format when dev->ref_count = %d",
                dev->ref_count);
    ret = U_FAIL;
  }
  ...
  if (ret == U_SUCC && uffs_BufIsAllFree(dev) == U_FALSE && !force) {
...
static void DumpBufHex(struct uffs_DeviceSt *dev, const u8 *buf, int len,
                       dump_msg_cb *dump) {
  int i;
  for (i = 0; i < len; i++)
    dump(dev, "%02X ", buf[i]);
...
static int DumpTag(struct uffs_DeviceSt *dev, int block, int page,
                   uffs_Tags *tag, dump_msg_cb *dump) {
  ...
  dump(dev, " - page %2d/%2d %s %d/%d len%4d\n", page, s->page_id,
       GetTagName(s), s->serial, s->parent, s->data_len);
