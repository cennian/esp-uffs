...
#define GET_OBJ_NODE_SERIAL(obj)                                               \
  ((obj)->type == UFFS_TYPE_DIR ? (obj)->node->u.dir.serial                    \
                                : (obj)->node->u.file.serial)

#define GET_OBJ_NODE_FATHER(obj)                                               \
  ((obj)->type == UFFS_TYPE_DIR ? (obj)->node->u.dir.parent                    \
...
#define GET_BLOCK_FROM_NODE(obj)                                               \
  ((obj)->type == UFFS_TYPE_DIR ? (obj)->node->u.dir.block                     \
...
static int _object_data[(sizeof(struct uffs_ObjectSt) * MAX_OBJECT_HANDLE) /
                        sizeof(int)];

static uffs_Pool _object_pool;

uffs_Pool *uffs_GetObjectPool(void) { return &_object_pool; }
 ...
 */
URET uffs_ReleaseObjectBuf(void) { return uffs_PoolRelease(&_object_pool); }
...
URET uffs_CreateObject(uffs_Object *obj, const char *fullname, int oflag) {
  ...
  if (uffs_ParseObject(obj, fullname) == U_SUCC)
    uffs_CreateObjectEx(obj, obj->dev, obj->parent, obj->name, obj->name_len,
                        oflag);
 ...
 */
URET uffs_CreateObjectEx(uffs_Object *obj, uffs_Device *dev, int dir,
                         const char *name, int name_len, int oflag) {
  uffs_Buf *buf = NULL;
  uffs_FileInfo fi;
  TreeNode *node;
  ...
  if (obj->type == UFFS_TYPE_DIR) {
    ...
    node = uffs_TreeFindFileNodeByName(obj->dev, obj->name, obj->name_len,
                                       obj->sum, obj->parent);
    if (node != NULL) {
    ...
    obj->node = uffs_TreeFindDirNodeByName(obj->dev, obj->name, obj->name_len,
                                           obj->sum, obj->parent);
    if (obj->node != NULL) {
    ...
    node = uffs_TreeFindDirNodeByName(obj->dev, obj->name, obj->name_len,
                                      obj->sum, obj->parent);
    if (node != NULL) {
      obj->err = UEEXIST;
      goto ext_1;
    }
    obj->node = uffs_TreeFindFileNodeByName(obj->dev, obj->name, obj->name_len,
                                            obj->sum, obj->parent);
    if (obj->node) {
  ...
  if (obj->dev->tree.erased_count < obj->dev->cfg.reserved_free_blocks) {
    uffs_Perror(UFFS_MSG_NOISY, "insufficient block in create obj");
    obj->err = UENOMEM;
    goto ext_1;
  }
  ...
  if (buf == NULL) {
    uffs_Perror(UFFS_MSG_SERIOUS, "Can't create new buffer when create obj!");
    goto ext_1;
  }

  memset(&fi, 0, sizeof(uffs_FileInfo));
  fi.name_len =
      obj->name_len < sizeof(fi.name) ? obj->name_len : sizeof(fi.name) - 1;
 ...
 * \return U_SUCC or U_FAIL (error code in obj->err).
 */
URET uffs_OpenObjectEx(uffs_Object *obj, uffs_Device *dev, int dir,
                       const char *name, int name_len, int oflag) {
  ...
  if ((oflag & (UO_WRONLY | UO_RDWR)) == (UO_WRONLY | UO_RDWR)) {
    ...
    uffs_Perror(UFFS_MSG_NOISY, "UO_WRONLY and UO_RDWR can't appear together");
    obj->err = UEINVAL;
    goto ext;
  }
...
URET uffs_OpenObject(uffs_Object *obj, const char *name, int oflag) {
  ...
  if ((ret = uffs_ParseObject(obj, name)) == U_SUCC) {
    ret = uffs_OpenObjectEx(obj, obj->dev, obj->parent, obj->name,
                            obj->name_len, oflag);
  }
  if (ret != U_SUCC)
...
static URET do_FlushObject(uffs_Object *obj) {
  ...
  if (obj->node) {
    ...
    if (obj->type == UFFS_TYPE_DIR)
      ...
      ret = (uffs_BufFlushGroupMatchParent(dev, obj->node->u.file.serial) ==
                 U_SUCC &&
             uffs_BufFlushGroup(dev, obj->node->u.file.parent,
                                obj->node->u.file.serial) == U_SUCC)
                ? U_SUCC
                : U_FAIL;
    }
...
static u32 GetStartOfDataBlock(uffs_Object *obj, u16 fdn) {
  if (fdn == 0) {
    ...
    return (obj->head_pages * obj->dev->com.pg_data_size) +
           (fdn - 1) *
               (obj->dev->com.pg_data_size * obj->dev->attr->pages_per_block);
  }
}

static int do_WriteNewBlock(uffs_Object *obj, const void *data, u32 len,
  ...
  for (page_id = 0; page_id < dev->attr->pages_per_block; page_id++) {
    size = (len - wroteSize) > dev->com.pg_data_size ? dev->com.pg_data_size
                                                     : len - wroteSize;
    if (size <= 0)
      break;
    ...
    ret = uffs_BufWrite(dev, buf, data == NULL ? NULL : (u8 *)data + wroteSize,
                        0, size);
    uffs_BufPut(dev, buf);
...
static int do_WriteInternalBlock(uffs_Object *obj, TreeNode *node, u16 fdn,
                                 const void *data, u32 len, u32 blockOfs) {
  uffs_Device *dev = obj->dev;
  u16 maxPageID;
  u16 page_id;
  u32 size;
    ...
    if (write_start == fnode->u.file.len && fdn > 0 &&
      ...
      if (dev->tree.erased_count < dev->cfg.reserved_free_blocks) {
        uffs_Perror(UFFS_MSG_NOISY,
                    "insufficient block in write obj, new block");
        break;
      }
      ...
      //
      // Flush the new block buffers immediately, so that the new data node will
      // be created and put in the tree.
      //
      ...
      size = do_WriteInternalBlock(
          obj, dnode, fdn, data ? (u8 *)data + len - remain : NULL, remain,
          write_start - GetStartOfDataBlock(obj, fdn));
...
int uffs_WriteObject(uffs_Object *obj, const void *data, int len) {
  ...
  if (obj->oflag & UO_APPEND)
    ...
    if (obj->pos > fnode->u.file.len) {
      ...
      remain = do_WriteObject(
          obj, NULL,
          pos - fnode->u.file
                    .len); // Write filling bytes. Note: the filling data does
                           // not count as 'wrote' in this write operation.
      obj->pos = pos - remain;
...
int uffs_ReadObject(uffs_Object *obj, void *data, int len) {
  ...
  while (remain > 0) {
    ...
    size =
        (remain + pageOfs > buf->data_len ? buf->data_len - pageOfs : remain);
...
// We need to discard one or more pages within this block, hence requires 'block
// recover'.
//
static URET do_TruncateInternalWithBlockRecover(uffs_Object *obj, u16 fdn,
                                                u32 remain,
                                                RunOptionE run_opt) {
  uffs_Device *dev = obj->dev;
  TreeNode *fnode = obj->node;
  ...
  if (fdn == 0) {
    ...
    if (node == NULL) {
      obj->err = UEIOERR;
      uffs_Perror(UFFS_MSG_SERIOUS, "can't find data node when truncate obj");
      goto ext;
    }
    ...
    block_start =
        obj->head_pages * dev->com.pg_data_size +
        (fdn - 1) * dev->com.pg_data_size * dev->attr->pages_per_block;
    parent = node->u.data.parent;
    serial = node->u.data.serial;
  }

  if (run_opt == eDRY_RUN) {
    // checking the buffer. this is the main reason why we need the 'dry run'
    // mode.
  ...
  if (!uffs_Assert(page_id <= max_page_id,
                   "fdn = %d, block_start = %d, remain = %d\n", fdn,
                   block_start, remain)) {
    obj->err = UEUNKNOWN_ERR;
    goto ext;
  }
...
static URET do_TruncateObject(uffs_Object *obj, u32 remain,
                              RunOptionE run_opt) {
  uffs_Device *dev = obj->dev;
  TreeNode *fnode = obj->node;
  u16 fdn;
  ...
  if (flen < remain) {
    ...
    if (run_opt == eREAL_RUN) {
      ...
      if (do_WriteObject(obj, NULL, remain - flen) > 0) { // fill '\0' ...
        uffs_Perror(UFFS_MSG_SERIOUS,
                    "Write object not finished. expect %d but only %d wrote.",
    ...
    while (flen > remain) {
      ...
      if (remain <= block_start && fdn > 0) {
        ...
        if (do_TruncateInternalWithBlockRecover(obj, fdn, remain, run_opt) ==
            U_SUCC) {
          if (run_opt == eREAL_RUN)
...
int _CheckObjBufRef(uffs_Object *obj) {
  ...
  for (buf = uffs_BufFind(dev, obj->parent, obj->serial, UFFS_ALL_PAGES);
       buf != NULL; buf = uffs_BufFindFrom(dev, buf->next, obj->parent,
                                           obj->serial, UFFS_ALL_PAGES)) {
    if (buf->ref_count > 0) {
      // oops ...
      uffs_Perror(UFFS_MSG_SERIOUS,
                  "someone still hold buf parent = %d, serial = %d, ref_count",
    ...
    if (obj->type == UFFS_TYPE_FILE && node->u.file.len > 0) {
      ...
      for (serial = 1; serial <= last_serial; serial++) {
        ...
        for (buf = uffs_BufFind(dev, parent, serial, UFFS_ALL_PAGES);
             buf != NULL; buf = uffs_BufFindFrom(dev, buf->next, parent, serial,
                                                 UFFS_ALL_PAGES)) {
          if (buf->ref_count != 0) {
            // oops ...
            uffs_Perror(
...
URET uffs_DeleteObject(const char *name, int *err) {
  ...
  uffs_ObjectDevLock(obj);
  work = NULL;
  while ((work = (uffs_Object *)uffs_PoolFindNextAllocated(&_object_pool,
                                                           work)) != NULL) {
    if (work != obj && work->dev && work->dev == obj->dev && work->node &&
        work->node == obj->node) {
  ...
  parent = obj->serial;
  last_serial = (obj->type == UFFS_TYPE_FILE && node->u.file.len > 0
                     ? GetFdnByOfs(obj, node->u.file.len - 1)
                     : 0);
  ...
  if (obj->type == UFFS_TYPE_FILE && last_serial > 0) {
    for (serial = 1; serial <= last_serial; serial++) {
      ...
      if (uffs_Assert(d_node != NULL,
                      "Can't find DATA node parent = %d, serial = %d\n", parent,
                      serial)) {
 ...
 */
URET uffs_MoveObjectEx(uffs_Object *obj, int new_parent, const char *new_name,
                       int name_len) {
  uffs_Buf *buf;
  uffs_FileInfo fi;
  uffs_Device *dev = obj->dev;
 ...
 * \return U_SUCC if success, otherwise return U_FAIL and set error code to
 * *err.
...
URET uffs_RenameObject(const char *old_name, const char *new_name, int *err) {
  ...
  if (obj->dev != new_obj->dev) {
    ...
    ret = uffs_MoveObjectEx(obj, new_obj->parent, new_obj->name,
                            new_obj->name_len);
    if (ret == U_FAIL && err)
      *err = obj->err;
  }
