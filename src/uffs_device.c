...
void uffs_DeviceInitLock(uffs_Device *dev) {
  uffs_Perror(UFFS_MSG_NOISY, "[Device] InitLock dev=%p", dev);
...
void uffs_DeviceLock(uffs_Device *dev) {
  ...
  if (dev->lock.counter != 0) {
    uffs_Perror(UFFS_MSG_NORMAL, "Lock device, counter %d NOT zero?!",
                dev->lock.counter);
  }
...
void uffs_DeviceUnLock(uffs_Device *dev) {
  ...
  if (dev->lock.counter != 0) {
    uffs_Perror(UFFS_MSG_NORMAL, "Unlock device, counter %d NOT zero?!",
                dev->lock.counter);
  }
