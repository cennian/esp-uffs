 ...
 *			(before the block turn 'bad', it must belong to
 * something ...)
 */
...
static void process_pending_recover(uffs_Device *dev, uffs_PendingBlock *s) {
  ...
  if (bad == NULL) {
    uffs_Perror(UFFS_MSG_SERIOUS,
                "can't find the reported bad block(%d) in the tree ? probably "
...
retry:
  ...
  for (i = 0; i < dev->attr->pages_per_block; i++) {
    ...
    if (page == UFFS_INVALID_PAGE) {
      ...
      uffs_Perror(UFFS_MSG_SERIOUS,
                  "bad block recover (block %d) not finished!", bc->block);
      break;
    }
    tag = GET_TAG(bc, page);
    ...
    if (UFFS_FLASH_HAVE_ERR(ret)) {
      if (UFFS_FLASH_IS_BAD_BLOCK(ret)) {
        // NOTE: Since we are trying to recover data from a 'bad' block, we
        // can't guarantee the data is ECC ok. 		If data is corrupted we can't do
        //anything about it ...
        uffs_Perror(UFFS_MSG_NORMAL,
                    "Read block %d page %d, return bad block or ECC failure, "
                    "data corrupted!",
                    bc->block, page);
    ...
    // if good block info already been loaded then use it, otherwise use local
    // tag
    if (newBc) {
    ...
    *newTag = *tag;
    ...
    if (UFFS_FLASH_IS_BAD_BLOCK(ret)) {
      ...
      uffs_Perror(UFFS_MSG_NOISY,
                  "A new bad block is discovered during bad block recover ...");
  ...
  if (succRecov == U_TRUE) {
    ...
    uffs_Perror(UFFS_MSG_NOISY,
                "recovered bad block %d replaced by %d, type %d!", s->block,
                good->u.list.block, type);
...
void uffs_BadBlockRecover(uffs_Device *dev) {
  ...
  while (dev->pending.count > 0) {
    ...
    uffs_Perror(UFFS_MSG_NOISY, "Process pending block %d - %s", s->block,
                uffs_BadBlockPendingTypeName(s->mark));
...
void uffs_BadBlockAdd(uffs_Device *dev, int block, u8 mark) {
  ...
  for (i = 0; i < dev->pending.count; i++) {
    ...
    if (s->block == block) {
      ...
      if (s->mark < mark) { // RECOVER would overwrite REFRESH, MARKBAD would
                            // overwrite RECOVER/REFRESH
        s->mark = mark;
        uffs_Perror(UFFS_MSG_NOISY, "Change pending block %d - %s", block,
                    uffs_BadBlockPendingTypeName(s->mark));
      }
      return;
    }
  ...
  if (dev->pending.count >= CONFIG_MAX_PENDING_BLOCKS) {
    uffs_Perror(UFFS_MSG_SERIOUS, "Too many pending bad blocks, please "
                                  "increase CONFIG_MAX_PENDING_BLOCKS !");
    return;
  }
  ...
  s->block = block;
  s->mark = mark;
  uffs_Perror(UFFS_MSG_NOISY, "Add pending block %d - %s", block,
              uffs_BadBlockPendingTypeName(mark));
}

/** put a new block to the bad block pending list by flash operation result
 * (flash_op_ret) return the block pending type. return UFFS_PENDING_BLK_NONE if
 * the block not added to the pending list.
 */
int uffs_BadBlockAddByFlashResult(uffs_Device *dev, int block,
                                  int flash_op_ret) {
 ...
 * \return bad block pending list node pointer or NULL if block is not in the
 * pending list.
 */
 ...
 * \return U_SUCC if the block is removed from pending list, U_FAIL if no block
 * removed.
 */
URET uffs_BadBlockPendingRemove(uffs_Device *dev, int block) {
  ...
  if (i < dev->pending.count) {
    uffs_Perror(UFFS_MSG_NOISY, "pending block %d removed",
                dev->pending.list[i].block);
